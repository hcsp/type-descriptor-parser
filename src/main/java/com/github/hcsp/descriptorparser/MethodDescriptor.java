...
public class MethodDescriptor implements TypeDescriptor {
    ...
    private static final Pattern firstPattern = Pattern.compile("^\\((.*)\\)(.+)$");
    private static final Pattern secondPattern = Pattern.compile("(\\[*(?:[BCDFIJSZV]|L.*;))");

    public MethodDescriptor(String descriptor) {
        this.descriptor = descriptor;
        Matcher firstMatcher = firstPattern.matcher(descriptor);
        while (firstMatcher.find()) {
            String returnTypeDesc = firstMatcher.group(2);
            String paramTypesDesc = firstMatcher.group(1);
            Matcher secondMatcher = secondPattern.matcher(paramTypesDesc);
